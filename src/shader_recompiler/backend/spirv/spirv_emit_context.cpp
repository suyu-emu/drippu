...
void DefineConstBuffers(EmitContext& ctx, const Info& info, Id UniformDefinitions::*member_type,
    ...
    // Deduplicate each array/struct size before decorating it. SPIR-V types
    // can be interned, so decorating an equal-sized type twice is invalid.
    ...
    const auto block_pointer_type{[&](u32 elements) {
        ...
        if (it != block_types.end()) return it->second;
    ...
    for (const ConstantBufferDescriptor& desc : info.constant_buffer_descriptors) {
        // Match the uploaded range. Dynamic/global-memory accesses retain the
        // full guest range; the minimum array length remains one element.
        ...
        if (ctx.profile.supported_spirv >= 0x00010400) ctx.interfaces.push_back(id);
        binding += desc.count;
    }
}
