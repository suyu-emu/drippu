# suyu / drippu verification map

This directory is the maintained source for verifying the Qt desktop frontend. The binary, MCP `emulator` id, and XDG paths are still **suyu**; user-facing copy is partially **drippu**. Read the index before driving the app, then use the matching feature file as the recipe.
...
- Set `SUYU_MCP_TIMEOUT` to at least `60` when exercising `get_lobby_row_counts`.
...
- Modal tools (`settings` / `configure` / file pickers) can return an empty MCP body while the dialog is up — prove them with `screenshot --target active_modal` and the window title.
- Restore the gamer library view (`navigate_gamer_view {"view":"library"}`) after opening Settings, Multiplayer, or Manual. If a modal still blocks MCP, cleanup and relaunch.
- Cleanup removes the isolate. Leave proof under `$SUYU_VERIFY_EVIDENCE_DIR`.

## Proof and skip reporting

- Capture the user action and the resulting state, not only the final screen.
- UI proof includes MCP JSON and a PNG that shows window identity (`suyu` + `drippu` in the title chrome / Gamer sidebar).
