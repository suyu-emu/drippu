...
The Qt frontend presents three full-window layouts: Gamer (library), Programmer (IDE-style), and Hacker (debug dock plus MCP tools). The first launch asks which profile to use unless a flag or a remembered choice is set.
...
- `mode-selector-skipped` confirms unattended `-gamer`/`-hacker`/`-programmer` does not show **drippu | Setup Profile**.

## How to get to it (user POV)

- On a first launch without a remembered profile, pick **Gamer**, **Programmer**, or **Hacker** in **drippu | Setup Profile**.
...
- **Selector skipped.** The screenshot / window list must not show title `drippu | Setup Profile` (nor the obsolete `suyu | Setup Profile`). If it does, launch seeding failed; cleanup and relaunch.
...
- QSettings org/app names remain `suyu team` / `suyu`; that is why launch seeds those paths even though the selector title says drippu.
