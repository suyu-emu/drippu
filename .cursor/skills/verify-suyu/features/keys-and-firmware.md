...
- Choose **Tools → Install Decryption Keys** and pick `prod.keys` or a folder that contains it.
- Choose **Tools → Install Firmware** and pick a folder or ZIP of NCAs.
- Choose **File → Setup** for the checklist (keys, firmware, games, optional recompile).
- First-run **Welcome to suyu** also points at profile / Nintendo / Steam setup; launch seeding skips that window.
...
- **Keys dialog.** Run `control-suyu call trigger_ui_action '{"action":"install_keys_dialog"}'`. `success` is true. Screenshot `keys-dialog.png` with `--target active_modal` (file picker or flow dialog).
- **Firmware dialog.** After dismissing or on a fresh launch, run `control-suyu call trigger_ui_action '{"action":"install_firmware_dialog"}'`. Screenshot `firmware-dialog.png`.
...
- File pickers are native Qt dialogs. If a picker is still open, later MCP tools that grab `active_modal` will screenshot the picker, not the main window.
