...
Users change emulator and UI settings from a modal **suyu Configuration** dialog (General, System, CPU, Graphics, Audio, Controls) and can switch light/dark theme immediately. The in-app user manual is a separate window.
...
- `config-theme` switches light, dark, or auto and applies it without OK.
...
- Choose **More Options** in the Gamer sidebar (overflow).
- Choose **Help → User Manual**, or **Manual** in the Gamer sidebar.
- Theme is also reachable from UI settings inside the dialog; MCP `set_theme_mode` is the unattended equivalent of that control.
...
- **Open from Gamer.** Run `control-suyu call navigate_gamer_view '{"view":"settings"}'`. `success` is true. Run `control-suyu screenshot --target active_modal --path "$SUYU_VERIFY_EVIDENCE_DIR/config-dialog.png"`. The modal title is `suyu Configuration`.
...
- **Manual.** Run `control-suyu call trigger_ui_action '{"action":"open_user_manual"}'` or `navigate_gamer_view '{"view":"manual"}'`. Screenshot `manual.png` via `active_window` or `main_window` showing the manual content.
